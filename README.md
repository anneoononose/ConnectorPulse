...
Visualizes deployment health across all connectors in real-time
Accepts raw error logs or system alerts from any connector
Uses Claude AI to diagnose root cause in plain English (no log-reading expertise required)
Returns prioritized, self-serve resolution steps with estimated resolution time
Surfaces diagnostic commands admins can run to verify the fix
...
Why no framework? This is an admin tooling prototype. Keeping it dependency-free means it can be embedded in any existing admin console without build tooling.
---
Scenarios Covered
...
```bash
# Clone the repo
git clone https://github.com/anneoononose/ConnectorPulse

# Open in browser (no build step needed)
open index.html
```
The app calls the Anthropic API client-side. For production use, the API call should be proxied through a backend to protect credentials.
---
Roadmap (If Extended)
[ ] Connector-specific resolution playbooks — fine-tuned prompts per connector type (Salesforce vs. Zendesk vs. ServiceNow)
...
[ ] Slack/Teams alert integration — push AI diagnosis to admin channels on connector health drop
[ ] Proactive health scoring — predict connector failures before they impact users based on trend data
---
Context
Built as a portfolio project exploring enterprise admin tooling, inspired by patterns observed during a Product Operations internship at Splunk managing cross-functional delivery across 15+ teams. The problem of "operational knowledge that lives in the heads of experts, not in the product" is one of the highest-leverage areas for AI in enterprise SaaS.
---
Built by Anne Ononose · Computer Information Systems, University of Houston
